 ...
 */
/*
Plugin Name: Video.js - Custum Video Player
...
Author: <b>Zohaib Agio</b>, <a>Baqar Vigeo</a>
Version: 4.5.0
*/
...
//if($options['videojs_cdn'] == 'on') { //use the cdn hosted version
		//wp_register_script( 'videojs', 'http://vjs.zencdn.net/5.7.1/video.js' );
		//wp_register_script( 'videojs-ie', 'http://vjs.zencdn.net/ie8/1.1.2/videojs-ie8.min.js' );
    //wp_register_style( 'videojs', 'http://vjs.zencdn.net/5.7.1/video-js.css' );
    //wp_enqueue_style( 'videojs' );
    //wp_register_script( 'dash', plugins_url( 'videojs/dash.all.min.js' , __FILE__ ) );
    //wp_register_script( 'videojs-dash', plugins_url( 'videojs/videojs-dash.js' , __FILE__ ) );
//	} else { //use the self hosted version
		//wp_register_style( 'videojs-vast',  plugins_url( 'bin/videojs.vast.vpaid.min.css' , __FILE__ ) );
		//wp_register_style( 'videojs-dash',  plugins_url( 'libs/videojs-contrib-dash-2.0.0/src/css/videojs-dash.css' , __FILE__ ) );
		wp_register_style( 'videojs-bsongs-css', plugins_url( 'source/videojs.css' , __FILE__ ) );
		wp_register_style( 'ads', plugins_url( 'source/videojsads.css' , __FILE__ ) );
		wp_register_style( 'ima', plugins_url( 'source/videojsima.css' , __FILE__ ) );




// calling that css which is added up ther //
		wp_enqueue_style( 'videojs-bsongs-css' );
		wp_enqueue_style( 'ads' );
		wp_enqueue_style( 'ima' );

//	}
	//wp_register_script( 'video.js_vast', plugins_url( 'bin/videojs_5.vast.vpaid.min.js' , __FILE__ ) );
	wp_register_script( 'videojs-bsongs', plugins_url( 'source/5.7.1video.js' , __FILE__ ) );
	wp_register_script( 'contrib-dash', plugins_url( 'source/contribdash.js' , __FILE__ ) );
	wp_register_script( 'c-dash', plugins_url( 'source/cdash.js' , __FILE__ ) );
  wp_register_script( 'ads', plugins_url( 'source/adscript.js' , __FILE__ ) );
  wp_register_script( 'ima', plugins_url( 'source/ima.js' , __FILE__ ) );
	wp_register_script( 'ima3', 'http://imasdk.googleapis.com/js/sdkloader/ima3.js' );
	wp_register_script( 'videoplayer', plugins_url( 'libs/videoplayer.js' , __FILE__ ) );


	//wp_register_script( 'es5-shim', plugins_url( 'bin/es5-shim.js' , __FILE__ ) );
	//wp_register_script( 'ie8fix', plugins_url( 'bin/ie8fix.js' , __FILE__ ) );
	//wp_register_script( 'googledfp', plugins_url( 'bin/googledfp.js' , __FILE__ ) );
...
function add_videojs_header(){
	...
	wp_enqueue_script( 'videojs-bsongs' );
	wp_enqueue_script( 'ads' );
	wp_enqueue_script( 'ima' );
	wp_enqueue_script( 'ima3' );
	wp_enqueue_script( 'contrib-dash' );
	wp_enqueue_script( 'c-dash' );
	wp_enqueue_script( 'videoplayer' );
	//wp_enqueue_script( 'videojs-youtube' );
	//wp_enqueue_script( 'video.js_vast' );
	//wp_enqueue_script( 'es5-shim' );
	//wp_enqueue_script( 'ie8fix' );
	//wp_enqueue_script( 'googledfp' );
}
...
//testing function for video ads in ads//
...
function video_shortcode($atts, $content=null){
	...
	// MP4 Source Supplied
	if ($mpd)
		$mpd_source = '<source src="'.$mpd.'" type=\'application/dash+xml\' />';
	else
		$mpd_source = '';
	...
	<!-- Begin Video.js -->
	<video id="content_video" class="video-js vjs-default-skin{$class}" width="{$width}" height="{$height}"{$poster_attribute}{$controls_attribute}{$preload_attribute}{$autoplay_attribute}{$loop_attribute}{$muted_attribute} data-setup='{$jsonDataSetup}'>
		{$mpd_source}
		{$mp4_source}
		{$webm_source}
		{$ogg_source}{$track}
